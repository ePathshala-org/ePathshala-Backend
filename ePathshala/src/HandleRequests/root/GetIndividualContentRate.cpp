#include "GetIndividualContentRate.h"

void GetIndividualContentRate(Json::Value &request, Json::Value &response, drogon::orm::DbClient &dbClient)
{
    std::clog << "Get \"individual-content-rate\" request" << std::endl;

    std::stringstream queryStream("SELECT GET_INDIVIDUAL_CONTENT_RATE($1, $2) AS RETURN");
